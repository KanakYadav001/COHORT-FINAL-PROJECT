...
async function UpdateCart(req, res) {
    ...
    if(!productId || !qty ||  qty < 1){
        return res.status(400).json({message : "productId and quantity are required and quantity must be greater than 0"})
    }
    ...
    const existingItem = cartItem.item.find(item => item.productId.toString() === productId);
